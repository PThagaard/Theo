...
// Activity #7, "Spejl": the front camera as a mirror (Chromium's fake camera here). A tap puts a sticker down, a
// drawn finger leaves glitter, a still finger grows a star that bursts when let go, and a shake throws confetti.
await page.evaluate(() => { window.__theo.startActivity('spejl'); window.__theo.game.setAge('2+'); });
check((await page.evaluate(() => window.__theo.activity.id)) === 'spejl', 'the Spejl activity did not start');
await page.waitForFunction(() => window.__theo.activity.debug.hasVideo(), null, { timeout: 8000 }).catch(() => undefined);
const mirrorVideo = await page.evaluate(() => window.__theo.activity.debug.hasVideo());
console.log('mirror camera:', mirrorVideo);
check(mirrorVideo, 'the mirror should show the (fake) camera picture');
const mirror = await page.evaluate(() => ({ w: window.__theo.game.width, h: window.__theo.game.height }));
await page.touchscreen.tap(mirror.w * 0.3, mirror.h * 0.4);
await page.touchscreen.tap(mirror.w * 0.6, mirror.h * 0.5);
await page.waitForTimeout(150);
check(await page.evaluate(() => window.__theo.game.stickers.length === 2), 'two taps should put two stickers on the mirror');
await page.mouse.move(mirror.w * 0.2, mirror.h * 0.72);
await page.mouse.down();
await page.mouse.move(mirror.w * 0.8, mirror.h * 0.66, { steps: 14 });
await page.mouse.up();
check(await page.evaluate(() => window.__theo.game.glitter.length > 10), 'a drawn finger should leave glitter on the mirror');
await page.screenshot({ path: `${OUT}/${tag}-70-spejl.png` });
await page.mouse.move(mirror.w * 0.5, mirror.h * 0.3);
await page.mouse.down();
await page.waitForTimeout(900);
check(await page.evaluate(() => { const c = window.__theo.game.charge; return !!c && c.size > 0.2; }), 'a still finger should grow a star on the mirror');
await page.screenshot({ path: `${OUT}/${tag}-71-spejl-star.png` });
await page.mouse.up();
await page.waitForTimeout(100);
check(await page.evaluate(() => window.__theo.game.charge === null && window.__theo.game.bursts === 1), 'letting go should burst the star');
const mirrorShook = await page.evaluate(() => {
  const g = window.__theo.game; const before = g.shakes;
  const fire = (x, y, z) => window.dispatchEvent(new DeviceMotionEvent('devicemotion', { accelerationIncludingGravity: { x, y, z } }));
  fire(0, 9.8, 0); fire(18, -12, 6);
  return g.shakes - before;
...
await page.waitForTimeout(400);
const mirrorStats = await page.evaluate(() => { const s = window.__theo.stats.snapshot.today; return { stickers: s.mirrorStickers ?? 0, trails: s.mirrorTrails ?? 0, bursts: s.mirrorBursts ?? 0, shakes: s.mirrorShakes ?? 0 }; });
console.log('spejl stats:', JSON.stringify(mirrorStats), 'shook', mirrorShook);
check(mirrorStats.stickers >= 3 && mirrorStats.trails >= 1 && mirrorStats.bursts >= 1 && mirrorShook === 1 && mirrorStats.shakes >= 1, 'Spejl did not answer tap, draw, hold and shake');
